    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                ...
                <div class="row">
                      <div class="col-sm-12">
                         Total Assesment: <span id="total-users">0</span>
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each(response.users, function(key, item) {
                ...
                const questionUrl = "{{ route('traineeViewQuestions') }}";
                ...
                if (item.question === 'NA' || item.question === null || item.question === '') {
                    questionLink =
                        '<a class="badge bg-info" ' +
                        'href="' + questionUrl + '?exam_id=' + item.id + '" target="_blank">(Click to View/attempt)</a>';
                        actionLink=
                         '<a class="badge bg-info" ' +
                        'href="' + questionUrl + '?exam_id=' + item.id + '" target="_blank"> (Click to View/attempt)</a>';

                        score=item.student_multiple_choice_score
                        student_work='<a class="badge bg-info" ' +
                        'href="' + questionUrl + '?exam_id=' + item.id + '" target="_blank"> (Click to View/attempt)</a>';
                } else {
                    questionLink =
                        ...
                        actionLink=
                        '<a href="#"><span  class="badge bg-success updateBtn" href="#" data-id="' + item.id + '"><i class="uil-trash"></i> Upload Your Work</span></a>';
                          score = item.student_score + ' / ' + item.marks;

                          student_work='<a href="{{ asset('practicals') }}/' + item.student_answer + '" download>' + item.student_answer + ' (Download)</a>'
                    }
                ...
                $('#table1').append(
                    '<tr>\
                        ...
                        <td>'+ actionLink +'</td>\
                        <!--<td> <a href="#"><span  class="badge bg-success updateBtn" href="#" data-id="' + item.id + '"><i class="uil-trash"></i> Upload Your Work</span></a>\</td>-->\
                    </tr>'
                );
            });
